...
//如果滑鼠游標在男生身上，被指到的男生以及女主都會停住，其他的男生則會繼續走動，man_stop=0代表沒有指到男生，1~9則分別代表9位男生(6位普、3位帥)。
//當滑鼠游標不在男生身上，則會根據游標離女主的距離進行走或跑的動作。
void CGameStateRun::MainGirlMove() {
	if (p.x > n1[floor * 2 - 2].ManState[0].GetLeft() && p.x < n1[floor * 2 - 2].ManState[0].GetLeft() + 65 && p.y > 150 && p.y < 330) {
		man_stop = 1;
		main_girl[0].SetTopLeft(main_girl[2].GetLeft(), 250);
	...
	else if (p.x > n1[floor * 2 - 1].ManState[0].GetLeft() && p.x < n1[floor * 2 - 1].ManState[0].GetLeft() + 65 && p.y > 150 && p.y < 330) {
		man_stop = 2;
		main_girl[0].SetTopLeft(main_girl[2].GetLeft(), 250);
		if (p.x > main_girl[2].GetLeft()) {
			maingirl_state = 6;
	...
	else if (p.x > n3[floor * 2 - 1].ManState[0].GetLeft() && p.x < n3[floor * 2 - 1].ManState[0].GetLeft() + 65 && p.y > 150 && p.y < 330) {
		man_stop = 3;
		main_girl[0].SetTopLeft(main_girl[2].GetLeft(), 250);
		if (p.x > main_girl[2].GetLeft()) {
			maingirl_state = 6;
	...
	else if (p.x > s1.ManState[0].GetLeft() && p.x < s1.ManState[0].GetLeft() + 65 && p.y > 150 && p.y < 330) {
		man_stop = 4;
		main_girl[0].SetTopLeft(main_girl[2].GetLeft(), 250);
		if (p.x > main_girl[2].GetLeft()) {
			maingirl_state = 6;
	...
	else if (p.x > s2.ManState[0].GetLeft() && p.x < s2.ManState[0].GetLeft() + 65 && p.y > 150 && p.y < 330) {
		man_stop = 5;
		main_girl[0].SetTopLeft(main_girl[2].GetLeft(), 250);
		if (p.x > main_girl[2].GetLeft()) {
			maingirl_state = 6;
	...
	else if (p.x > s3.ManState[0].GetLeft() && p.x < s3.ManState[0].GetLeft() + 65 && p.y > 150 && p.y < 330) {
		man_stop = 6;
		main_girl[0].SetTopLeft(main_girl[2].GetLeft(), 250);
		if (p.x > main_girl[2].GetLeft()) {
			maingirl_state = 6;
	...
	else if (p.x > n2[floor * 2 - 2].ManState[0].GetLeft() && p.x < n2[floor * 2 - 2].ManState[0].GetLeft() + 65 && p.y > 350 && p.y < 530) {
		man_stop = 7;
		main_girl[0].SetTopLeft(main_girl[2].GetLeft(), 250);
		if (p.x > main_girl[2].GetLeft()) {
			maingirl_state = 6;
	...
	else if (p.x > n3[floor * 2 - 2].ManState[0].GetLeft() && p.x < n3[floor * 2 - 2].ManState[0].GetLeft() + 65 && p.y > 350 && p.y < 530) {
		man_stop = 8;
		main_girl[0].SetTopLeft(main_girl[2].GetLeft(), 250);
		if (p.x > main_girl[2].GetLeft()) {
			maingirl_state = 6;
	...
	else if (p.x > n2[floor * 2 - 1].ManState[0].GetLeft() && p.x < n2[floor * 2 - 1].ManState[0].GetLeft() + 65 && p.y > 350 && p.y < 530) {
		man_stop = 9;
		main_girl[0].SetTopLeft(main_girl[2].GetLeft(), 250);
		if (p.x > main_girl[2].GetLeft()) {
			maingirl_state = 6;
...
void CGameStateRun::OnShow() {
	...
	if (man_stop != 1) {
		n1[floor * 2 - 2].ShowMan(500, 800, map.GetLeft(), maingirl_state, 0, main_girl[2].GetLeft());
	}
	else {
		n1[floor * 2 - 2].ShowMan(500, 800, map.GetLeft(), maingirl_state, 1, main_girl[2].GetLeft());
	}
	if (man_stop != 2) {
		n1[floor * 2 - 1].ShowMan(1150, 1300, map.GetLeft(), maingirl_state, 0, main_girl[2].GetLeft());
	}
	else {
		n1[floor * 2 - 1].ShowMan(500, 800, map.GetLeft(), maingirl_state, 1, main_girl[2].GetLeft());
	}
	if (man_stop != 3) {
		n3[floor * 2 - 1].ShowMan(1950, 2200, map.GetLeft(), maingirl_state, 0, main_girl[2].GetLeft());
	}
	else {
		n3[floor * 2 - 1].ShowMan(500, 800, map.GetLeft(), maingirl_state, 1, main_girl[2].GetLeft());
	}
	if (man_stop != 4 && floor == 2) {
		s1.ShowMan(1600, 1700, map.GetLeft(), maingirl_state, 0, main_girl[2].GetLeft());
	}
	else if(floor == 2) {
		s1.ShowMan(500, 800, map.GetLeft(), maingirl_state, 1, main_girl[2].GetLeft());
	}
	if (man_stop != 5 && floor == 3) {
		s2.ShowMan(1600, 1700, map.GetLeft(), maingirl_state, 0, main_girl[2].GetLeft());
	}
	else if (floor == 3) {
		s2.ShowMan(500, 800, map.GetLeft(), maingirl_state, 1, main_girl[2].GetLeft());
	}
	if (man_stop != 6 && floor == 4) {
		s3.ShowMan(1600, 1700, map.GetLeft(), maingirl_state, 0, main_girl[2].GetLeft());
	}
	else if (floor == 4) {
		s3.ShowMan(500, 800, map.GetLeft(), maingirl_state, 1, main_girl[2].GetLeft());
	}
	...
	//圖層問題，故把這三個物件show在maingirl_state下面
	if (man_stop != 7) {
		n2[floor * 2 - 2].ShowMan(750, 1150, map.GetLeft(), maingirl_state, 0, main_girl[2].GetLeft());
	}
	else {
		n2[floor * 2 - 2].ShowMan(750, 1150, map.GetLeft(), maingirl_state, 1, main_girl[2].GetLeft());
	}
	if (man_stop != 8) {
		n3[floor * 2 - 2].ShowMan(1350, 1650, map.GetLeft(), maingirl_state, 0, main_girl[2].GetLeft());
	}
	else {
		n3[floor * 2 - 2].ShowMan(750, 1150, map.GetLeft(), maingirl_state, 1, main_girl[2].GetLeft());
	}
	if (man_stop != 9) {
		n2[floor * 2 - 1].ShowMan(1700, 1900, map.GetLeft(), maingirl_state, 0, main_girl[2].GetLeft());
	}
	else {
