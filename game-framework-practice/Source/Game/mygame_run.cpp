...
{
	if (A.Top()>=(B.Top() - 60) && A.Top()<= (B.Top() + 60) && A.Left() >= (B.Left() - 60) && A.Left() <= (B.Left() + 60)) {
		return true;
	}
	return false;
}
...
void CGameStateRun::OnKeyDown(UINT nChar, UINT nRepCnt, UINT nFlags)
{
	...
	if (CGameStateRun::OnMove(character, chest_and_key) && phase == 3) {
		chest_and_key.SelectShowBitmap(1);
	}
	if (phase == 5) {
		for (int i = 0; i < 3; i++) {
			if (CGameStateRun::OnMove(character, door[i])) {
